...
import { getUniqueYearsAndCountries, fetchMedalTally } from './utils';
...
const MedalTallyTable = () => {
  ...
  const { years, countries } = getUniqueYearsAndCountries(jsonData);

  useEffect(() => {
    setTableData(fetchMedalTally(jsonData, selectedYear, selectedCountry));
        ...
        {tableData.length > 0 ? (
          ...
          tableData.map((row, index) => (
            ...
            <View key={index} style={styles.row}>
              ...
              {/* <Text style={[styles.cell, styles.column1, topViewStyle.column1]}>{row.region}</Text> */}
